...
var lower_data = data.map(function (s) { return s.toLowerCase(); });
var found_data = [];
...
var Quiz = {
    ...
    createCounter: function () {
        ...
        answer_span.innerHTML = found_data.length;
        total_span.innerHTML = '/' + data.length;
        counter_div.appendChild(answer_span);
        counter_div.appendChild(total_span);
    },
    
    updateCounter: function () {
        document.getElementById('answer-count').innerHTML = found_data.length;
    },
    ...
    getText: function () {
        var text = document.getElementById('input').value.toLowerCase().trim();
        if (lower_data.indexOf(text) > -1) {
            var index = lower_data.indexOf(text);
            document.getElementById('input').value = '';
            var out_text = data.splice(index, 1)[0];
            found_data.push(lower_data.splice(index, 1)[0]);
            this.putText(out_text, document.getElementById('answers'));
        }
    },
    
    putText: function (text, table) {
        if (table.rows[table.rows.length - 1].cells.length < 4) {
            this.createCell(text, table);
        } else {
    ...
    createCell: function (text, table) {
        var new_cell = document.createElement('td');
        new_cell.innerHTML = text;
        table.rows[ table.rows.length - 1].appendChild(new_cell);
        this.updateCounter();
    },
    
    countTo: new Date(Date.now() + 300000),
    
    countdown: function () {
        if (Quiz.countTo >= Date.now() && Quiz.endCountdown === false) {
            document.getElementById('timer').innerHTML = countdown(Quiz.countTo).toString();
            requestAnimationFrame(Quiz.countdown);
        } else if (Quiz.endCountdown === true) {
            document.getElementById('timer').innerHTML = countdown(Quiz.countTo).toString();
        } else {
    ...
    shareScore: function () {
        ...
        var correct = found_data.length;
        var total = found_data.length + data.length;
        share.innerHTML = '<p><strong>'
						+ 'You got ' 
                        + correct
                        + ' out of ' 
                        + total
						+ '! Share your score!</strong> <a target="_blank" href="http://www.facebook.com/sharer/sharer.php?u=' 
                        ...
                        + '</p>';
        document.getElementById('score').appendChild(share);
    },
	...
	showAnswers: function () {
		...
		var len = data.length;
		for (i; i < len; i++) {
			this.putText(data[i], document.getElementById('unanswers'));
		}
	}
